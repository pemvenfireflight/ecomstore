    ...
    <div className="min-h-screen bg-white text-zinc-900 dark:bg-black dark:text-zinc-100">
      <main className="mx-auto flex w-full max-w-[1400px] flex-col gap-12 px-4 py-8 md:gap-16 md:px-8 md:py-12">
        <section className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
          <div className="flex flex-col justify-between rounded-3xl border border-zinc-200 bg-zinc-50 p-8 md:p-12 dark:border-zinc-800 dark:bg-zinc-950/60">
            <div className="space-y-5">
              <p className="text-[11px] font-medium tracking-[0.22em] text-zinc-500 uppercase">Spring 2026 Collection</p>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Link
                href="#shop"
                className="rounded-full bg-zinc-900 px-5 py-2.5 text-xs font-medium tracking-[0.14em] text-white uppercase transition hover:bg-zinc-700 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300"
              >
                Shop New Arrivals
              </Link>
              <Link
                href="/checkout"
                className="rounded-full border border-zinc-300 px-5 py-2.5 text-xs font-medium tracking-[0.14em] uppercase transition hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900"
              >
                View Cart
              </Link>
            </div>
          ...
          <div className="rounded-3xl border border-zinc-200 bg-[radial-gradient(circle_at_20%_20%,#ffffff,transparent_40%),linear-gradient(135deg,#111827,#52525b)] p-8 text-white md:p-10 dark:border-zinc-700">
            <div className="flex h-full flex-col justify-between">
              ...
              <div className="mt-8 grid grid-cols-2 gap-3 text-[11px] tracking-[0.12em] uppercase md:grid-cols-3">
                {stylePills.map((pill) => (
                  <span key={pill} className="rounded-full border border-white/30 bg-white/10 px-3 py-2 text-center">
                    {pill}
                  </span>
                ))}
              </div>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {promoTiles.map((tile) => (
            <article
              key={tile.title}
              className="space-y-2 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950/60"
            >
        ...
        <section id="shop" className="space-y-6">
          <div className="flex items-end justify-between gap-4 border-b border-zinc-200 pb-4 dark:border-zinc-800">
            <div>
              <p className="text-[11px] font-medium tracking-[0.2em] text-zinc-500 uppercase">Shop</p>
